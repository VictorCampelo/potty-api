"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CreatePlanDto = void 0;
const openapi = require("@nestjs/swagger");
const class_validator_1 = require("class-validator");
class CreatePlanDto {
    static _OPENAPI_METADATA_FACTORY() {
        return { name: { required: true, type: () => String }, qtd_products: { required: false, type: () => Number }, nickname: { required: true, type: () => String }, url: { required: true, type: () => String }, code: { required: true, type: () => Number } };
    }
}
__decorate([
    (0, class_validator_1.IsNotEmpty)({
        message: 'Insira o nome do plano',
    }),
    __metadata("design:type", String)
], CreatePlanDto.prototype, "name", void 0);
__decorate([
    (0, class_validator_1.IsNotEmpty)({
        message: 'Insira um nome curto para o plano',
    }),
    __metadata("design:type", String)
], CreatePlanDto.prototype, "nickname", void 0);
__decorate([
    (0, class_validator_1.IsNotEmpty)({
        message: 'Insira a URL de referência na Eduzz',
    }),
    __metadata("design:type", String)
], CreatePlanDto.prototype, "url", void 0);
__decorate([
    (0, class_validator_1.IsNotEmpty)({
        message: 'Insira o código do produto',
    }),
    __metadata("design:type", Number)
], CreatePlanDto.prototype, "code", void 0);
exports.CreatePlanDto = CreatePlanDto;
//# sourceMappingURL=create-plan.dto.js.map